...
///
/// Creates:
///   - A fresh SPL mint with `token_state` PDA as mint_authority (no freeze_authority)
...
pub struct InitializeMint<'info> {
    ...
    ///
    /// Freeze authority is intentionally NOT set. The LPToken EVM contract does
    /// not use any token-freezing functionality, and setting freeze_authority
    /// would grant unnecessary privilege. If freeze capability is needed in the
    /// future, a program upgrade can add it via set_authority CPI.
    #[account(
        init,
        payer = payer,
