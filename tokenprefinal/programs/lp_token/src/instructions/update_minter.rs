...
pub struct UpdateMinter<'info> {
    ...
    ///
    /// Safety: init_if_needed is safe here because PDA accounts can only be
    /// created by the owning program via system_program.create_account with
    /// PDA signer seeds. An attacker cannot front-run or pre-create this
    /// account. If the account already exists, Anchor validates the
    /// discriminator before deserialization, preventing data injection.
    #[account(
        init_if_needed,
        payer = owner,
...
impl UpdateMinter<'_> {
    pub fn apply(ctx: &mut Context<UpdateMinter>, params: &UpdateMinterParams) -> Result<()> {
        ...
        emit!(events::MinterUpdated {
            ...
            token_state: ctx.accounts.token_state.key(),
        });
