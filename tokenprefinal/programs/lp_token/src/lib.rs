...
pub use errors::LPTokenError;
pub use state::{MinterRecord, TokenState};
use instructions::*;

// TODO: Replace with mainnet program ID before deployment.
...
pub mod lp_token {
    ...
    /// Propose an ownership transfer to a new address (step 1 of 2).
    ///
    /// EVM: OwnableUpgradeable.transferOwnership(address newOwner)
    ///   - Only current owner may call
    ///
    /// Production improvement: uses two-step propose/accept pattern.
    /// The new owner must call `accept_ownership` to finalize.
    ...
    ///
    /// No direct EVM equivalent — production security improvement.
    ...
    /// Create or update on-chain token metadata (name, symbol, uri).
    ///
    /// EVM: name() and symbol() are stored at initialization.
    /// On Solana, SPL mints do not store metadata natively.
    /// This instruction uses Metaplex Token Metadata CPI to attach
    /// human-readable metadata to the mint.
    ///
    /// Access control: only the owner may call this.
    pub fn set_metadata(
        mut ctx: Context<SetMetadata>,
        params: SetMetadataParams,
    ) -> Result<()> {
        SetMetadata::apply(&mut ctx, &params)
    }
