...
/// Includes token_state so indexers can distinguish between multiple mints.
#[event]
...
/// Emitted when an ownership transfer is proposed (step 1 of 2).
...
/// Emitted when ownership transfer is finalized (step 2 of 2).
/// Maps to EVM: OwnershipTransferred(address indexed previousOwner, address indexed newOwner)
#[event]
...
/// Emitted when token metadata is created or updated via Metaplex.
#[event]
