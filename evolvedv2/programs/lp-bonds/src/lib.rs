...
declare_id!("7oFXPveRzDZUHSTxaRquLDn8Z7U3CZYLonyET13Sqaxe");
...
pub mod lp_bonds {
    ...
    pub fn add_liquidity_and_mint_bond(
        ...
        // Validate token vaults against whirlpool on-chain data (offset-based read)
        {
            let wp_data = ctx.accounts.whirlpool.try_borrow_data()?;
            require!(wp_data.len() >= 245, LpBondsError::InvalidAccountData);
            require!(
            ...
            let vault_a = Pubkey::try_from(&wp_data[133..165])
                .map_err(|_| LpBondsError::InvalidAccountData)?;
            let vault_b = Pubkey::try_from(&wp_data[213..245])
                .map_err(|_| LpBondsError::InvalidAccountData)?;
            require_keys_eq!(ctx.accounts.token_vault_a.key(), vault_a, LpBondsError::InvalidTokenVault);
            require_keys_eq!(ctx.accounts.token_vault_b.key(), vault_b, LpBondsError::InvalidTokenVault);
        }
